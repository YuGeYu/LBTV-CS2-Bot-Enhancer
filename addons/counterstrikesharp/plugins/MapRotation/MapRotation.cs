...
public sealed class MapRotationPlugin : BasePlugin
{
    ...
    public override string ModuleAuthor => "LBTV";
    public override string ModuleDescription => "Optional LBTV fixed map rotation controller.";
    ...
    private const float BotHiderSafeMapChangeDelaySeconds = 18.5f;
    private const float MissingWinPanelFallbackDelaySeconds = 30.0f;
    ...
    private bool _matchEndDetected;
    private bool _winPanelMatchSeen;
    private bool _missingWinPanelFallbackScheduled;
    ...
    public override void Load(bool hotReload)
    {
        ...
        _winPanelMatchSeen = false;
        _missingWinPanelFallbackScheduled = false;
        _scheduleSerial = 0;
        ...
        AddCommand("lbtv_map_rotation", "Show or toggle LBTV map rotation: lbtv_map_rotation [0|1]", OnMapRotationCommand);
        AddCommand("lbtv_map_next", "Change to the next map in the LBTV rotation.", OnMapNextCommand);
    ...
    private void EvaluateRoundEndAfterScoreUpdate(int evaluationSerial, int winner, int reason, string message)
    {
        ...
        MarkMatchEndDetected($"final-round detection: {matchState.DetectionReason}", matchState);
    }
    ...
    private HookResult OnGameEnd(EventGameEnd @event, GameEventInfo info)
    {
        ...
        SendEndMatchThanksOnce();
        MarkMatchEndDetected($"EventGameEnd fallback winner={@event.Winner}", matchState);
    ...
    {
        var currentMap = GetCurrentMap();
        Log($"EventCsWinPanelMatch fired. current={currentMap}, enabled={BoolToInt(_enabled)}, matchEndDetected={BoolToInt(_matchEndDetected)}, scheduled={BoolToInt(_changeScheduled)}");

        _matchEndDetected = true;
        _winPanelMatchSeen = true;
        SendEndMatchThanksOnce();

        ScheduleNextMap("EventCsWinPanelMatch after BotHider cleanup window", BotHiderSafeMapChangeDelaySeconds);
    ...
    private void OnMapRotationCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            command.ReplyToCommand($"{Prefix} Usage: lbtv_map_rotation [0|1]");
            return;
        }
        ...
        Log($"Command lbtv_map_rotation by {DescribeCaller(player, command)}: old={BoolToInt(oldState)}, new={BoolToInt(_enabled)}");
        ReplyStatus(command);
    }
    ...
    private void OnMapNextCommand(CCSPlayerController? player, CommandInfo command)
    {
        if (_changeScheduled)
        {
            ...
            Log($"Command lbtv_map_next by {DescribeCaller(player, command)} ignored: change already scheduled.");
            return;
        }
        ...
        Log($"Command lbtv_map_next by {DescribeCaller(player, command)}: current={currentMap}, next={nextMap}");
    ...
    private void MarkMatchEndDetected(string reason, MatchState matchState)
    {
        _matchEndDetected = true;
        Log($"match end detected, waiting for EventCsWinPanelMatch before rotating: reason={reason}, score={matchState.HighScore}-{matchState.LowScore}, target={matchState.RequiredWinScore}");
        ScheduleMissingWinPanelFallback(reason);
    }

    private void ScheduleMissingWinPanelFallback(string reason)
    {
        var currentMap = GetCurrentMap();

        if (!_enabled)
        {
            Log($"Not scheduling missing-win-panel fallback because rotation is disabled. reason={reason}, current={currentMap}");
            return;
        }

        if (_winPanelMatchSeen)
        {
            Log($"Not scheduling missing-win-panel fallback because EventCsWinPanelMatch already fired. reason={reason}, current={currentMap}");
            return;
        }

        if (_changeScheduled)
        {
            Log($"Not scheduling missing-win-panel fallback because changelevel is already scheduled. reason={reason}, current={currentMap}");
            return;
        }

        if (_missingWinPanelFallbackScheduled)
        {
            Log($"Not scheduling missing-win-panel fallback because one is already pending. reason={reason}, current={currentMap}");
            return;
        }

        var fallbackSerial = _scheduleSerial;
        _missingWinPanelFallbackScheduled = true;

        Log($"Scheduling missing-win-panel fallback by {reason}: current={currentMap}, delay={MissingWinPanelFallbackDelaySeconds:0.0}s, scheduleId={fallbackSerial}");
        AddTimer(MissingWinPanelFallbackDelaySeconds, () =>
        {
            _missingWinPanelFallbackScheduled = false;

            if (fallbackSerial != _scheduleSerial)
            {
                Log($"Skipping missing-win-panel fallback because map/schedule state changed. fallbackScheduleId={fallbackSerial}, activeScheduleId={_scheduleSerial}");
                return;
            }

            if (!_matchEndDetected || _winPanelMatchSeen || _changeScheduled)
            {
                Log($"Skipping missing-win-panel fallback: matchEndDetected={BoolToInt(_matchEndDetected)}, winPanelSeen={BoolToInt(_winPanelMatchSeen)}, scheduled={BoolToInt(_changeScheduled)}");
                return;
            }

            ScheduleNextMap($"missing EventCsWinPanelMatch fallback after {MissingWinPanelFallbackDelaySeconds:0.0}s: {reason}", delaySeconds: 0.0f);
        });
    }
    ...
    private void SendEndMatchThanksOnce()
    {
        ...
        Server.PrintToChatAll($"{LbtvPrefix} 感谢上游项目 ed0ard/CS2-Bot-Improver。");
        Server.PrintToChatAll($"{LbtvPrefix} 感谢所有提供帮助、测试和反馈的朋友们。");
    }
