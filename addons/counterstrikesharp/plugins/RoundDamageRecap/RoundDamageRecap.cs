...
public sealed class RoundDamageRecapPlugin : BasePlugin
{
    ...
    public override string ModuleDescription => "Shows an LBTV-style round-end damage recap in chat.";
    ...
    private void PrintRecapForPlayer(CCSPlayerController player)
    {
        ...
        foreach (var enemy in enemyPlayers)
        {
            ...
            PrintLbtvLine(
                player,
                $"{LbtvPrefix} To: [{toEntry.TotalDamage} / {toEntry.HitCount} hits] " +
                $"From: [{fromEntry.TotalDamage} / {fromEntry.HitCount} hits] - {enemy.PlayerName} [{remainingHp} hp]");
        }
    }
