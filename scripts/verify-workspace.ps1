...
if ($licenseContent -notmatch "GNU AFFERO GENERAL PUBLIC LICENSE") {
    $failures.Add("Root LICENSE must contain the GNU Affero General Public License v3.0 text.")
}
foreach ($readme in @("README.md", "README.zh-CN.md")) {
    $content = Get-Content -LiteralPath (Join-Path $repo $readme) -Raw
    foreach ($url in @("https://github.com/numakkiyu/CS2-Bot-Improver-Plus", "https://github.com/ed0ard/CS2-Bot-Improver")) {
        if (-not $content.Contains($url)) { $failures.Add("$readme is missing upstream link: $url") }
    }
}
...
$upstreamNote = Join-Path $repo "docs/UPSTREAM.md"
if (-not (Test-Path -LiteralPath $upstreamNote) -or
    (Get-Content -LiteralPath $upstreamNote -Raw) -notmatch [regex]::Escape($manifest.upstream.baseCommit)) {
    Add-Failure "docs/UPSTREAM.md does not identify the pinned Plus source commit."
}
...
$freezeHandlerStart = $nadeSystem.IndexOf("private HookResult OnFreezeEnd", [StringComparison]::Ordinal)
if ($freezeHandlerStart -lt 0 -or $nadeSystem.Substring($freezeHandlerStart, [Math]::Min(350, $nadeSystem.Length - $freezeHandlerStart)) -notmatch "SnapshotRoundUtilityBudgets\(\)") {
    Add-Failure "NadeSystem must create utility budgets only at freeze end."
}
foreach ($source in @('"replay"', '"instant"', '"retaliation"')) {
    if ($nadeSystem -notmatch "TryReserveUtility\([^\r\n]+$([regex]::Escape($source))") {
        Add-Failure "NadeSystem $source throw path does not use the unified reservation gate."
    }
}
if ($nadeSystem -notmatch 'NormalizeGrenadeType\(string type\)' -or
    $nadeSystem -notmatch 'incgrenade.*molotov' -or
    $nadeSystem -notmatch '"flash" => 2' -or
    $nadeSystem -notmatch '"smoke" or "he" or "molotov" => 1') {
    Add-Failure "NadeSystem per-bot utility limits or incgrenade/molotov normalization are missing."
}
foreach ($retired in @('BotTaunt', 'BotVision', 'MapRotation')) {
    if (Test-Path -LiteralPath (Join-Path $repo "addons/counterstrikesharp/plugins/$retired")) {
        Add-Failure "Retired LBTV plugin remains in the production source tree: $retired"
    }
}
...
if ($PackageRoot) {
    ...
    $packageLicense = Join-Path $package "LICENSE"
    if ((Test-Path -LiteralPath $packageLicense) -and
        ((Get-FileHash -LiteralPath $packageLicense -Algorithm SHA256).Hash -ne
         (Get-FileHash -LiteralPath $licensePath -Algorithm SHA256).Hash)) {
        Add-Failure "Package LICENSE does not match the repository root LICENSE."
    }
    $forbiddenPackageEntries = @(Get-ChildItem -LiteralPath $package -Recurse -Force | Where-Object {
        $_.Name -in @("vendor", "obj", ".cache", "node_modules", "artifacts") -or $_.Extension -in @(".cs", ".csproj", ".log")
    })
    if ($forbiddenPackageEntries.Count -gt 0) {
        Add-Failure "Package contains source, cache, or build artifacts: $($forbiddenPackageEntries.FullName -join ', ')"
    }
    ...
    $builtPlugins = @(
        # The release starts from the pinned Plus bundle. Only NadeSystem is
        # locally rebuilt and overlaid; the remaining runtime DLLs stay byte-for-byte upstream.
